...
{
    public class LandFogSystem
    {
        private const float Rate        = 40f;
        ...
        public LandFogSystem(Texture2D tex, Transform parent, float radius = 150f, float sizeMin = 30f, float sizeMax = 80f, float lifeMin = 10f, float lifeMax = 20f)
        {
            _radius = radius;
            _sizeMin = sizeMin;
            _sizeMax = sizeMax;
        ...
        public void Update(float dt, Vector3 camPos, float intensity, float alpha, float dayNight)
        {
            ...
            while (_budget >= 1f && spawned < MaxPerFrame)
            {
                ...
                Vector3 land = hit.point + Vector3.up * 0.05f;
