...
namespace NuclearRain
{
    public class SeaMistSystem
    {
        ...
        private const float SizeMin = 80f, SizeMax = 160f;
        ...
        public SeaMistSystem(Texture2D tex, Transform parent)
        {
            tex.wrapMode = TextureWrapMode.Clamp;
            var go = new GameObject("SeaMist");
        ...
        public void Update(float dt, Vector3 camPos, float camSpeed, float intensity, float alpha, float dayNight)
        {
            ...
            while (_budget >= 1f && spawned < MaxPerFrame)
            {
                                    ...
                                    QueryTriggerInteraction.Collide))
                {
                    ...
                    land = new Vector3(origin.x, Datum.LocalSeaY + 0.5f, origin.z);
                }
                else
                {
                    land = new Vector3(origin.x, Datum.LocalSeaY + 0.5f, origin.z);
