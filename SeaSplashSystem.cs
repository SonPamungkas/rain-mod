...
namespace NuclearRain
{
    public class SeaSplashSystem
    {
        ...
        private const float SizeMin = 1.4f, SizeMax = 3.2f;
        ...
        public void Update(float dt, Vector3 camPos, float intensity, float alpha, float dayNight)
        {
            ...
            while (_budget >= 1f && spawned < MaxPerFrame)
            {
                ...
                RainRaycast.TryFindGround(origin, ProbeDown, out Vector3 land, out bool isWater);
                if (!isWater) continue;
